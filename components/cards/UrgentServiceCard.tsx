// components/cards/UrgentServiceCard.tsx
"use client";

import { CardElement } from "@/components/ui/CardElement";
import {
  CardBadge,
  CardTitle,
  CardDescription,
  CardFeatureList,
  CardFooter,
} from "@/components/ui/card";
import { Button } from "@/components/ui/Button";

import { useBooking } from "@/contexts/BookingContext";

export function UrgentServiceCard() {
  const { openBooking } = useBooking();

  return (
    <CardElement className="p-6 pt-8 md:p-8 flex flex-col h-full">
      <CardBadge variant="dark">URGENTE</CardBadge>

      <CardTitle className="mt-6 text-[30px] font-semibold leading-[1.1]">
        Atendimento em menos de 24 horas
      </CardTitle>

      <CardDescription className="mt-3 font-body text-[16px] leading-[1.6] text-[#6B6B6B]">
        Esta opção foi criada para momentos em que a urgência emocional não pode
        esperar. Seja uma decisão importante, uma separação ou uma situação
        delicada, o pedido será cuidadosamente avaliado antes da confirmação,
        de acordo com as disponibilidades.
      </CardDescription>

      <CardFeatureList
        className="mt-4 text-[15px]"
        items={[
          "Sessão de uma hora",
          "Atendimento por telefone ou videoconferência",
          "Confirmação prévia obrigatória",
          "Troca confidencial, respeitosa e acolhedora",
        ]}
      />

      <div className="flex-1" />

      <Button
        variant="primary"
        size="lg"
        className="w-full text-[18px] normal-case mt-8 leading-[1]"
        onClick={() => {
          if (typeof window !== "undefined") {
            window.dataLayer = window.dataLayer || [];
            window.dataLayer.push({
              event: "booking_start",
              service_type: "urgent",
              service_category: "consultation",
            });
          }

          openBooking("urgent");
        }}
      >
        Pedido urgente — telefone / videoconferência
      </Button>

      <CardFooter className="mt-4 text-[14px] text-[#6B6B6B]">
        Disponibilidades limitadas
      </CardFooter>
    </CardElement>
  );
}